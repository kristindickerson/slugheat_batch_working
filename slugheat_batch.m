%% batch function
function slugheat_batch(yaml_file)
    %% 
    ...
    [loop,data_folder,penfile_type, hp_file]=assign_variables_from_yaml(yaml_file);
    ...
    if loop_pens
        ...
        inputfolder_choice = data_folder;
        % Specify the directory path
        folderPath = [pwd, '/batch_inputs/' inputfolder_choice];
    
        % Ask the user for file type
        filetype_choice = penfile_type;
        % Ask the user for presence heat pulse or not
        HP_choice = hp_file;
        ...
        varNamesBullRes = {'Cruise', 'Station', 'Pen', 'TrialNum', 'Iter','HP', 'BW','Sensor', 'Depths', 'EqTempsRelBW','BottomWaterTemp',  ...
        ...
        outputTableBullRes = array2table(zeros(0, numColsBullRes), 'VariableNames', varNamesBullRes);
        ...
        for i = 1:length(fileList)
            ...
            [HFData, ErrData, BullardResults]=batchmode(filePath, PulseData,folderPath,change_pars,par_file, counter);
    
            % Example 1x18 string array
            rowData = HFData; % Replace with your actual data
            rowBullRes = [repmat(HFData(1:7),str2num(HFData(8)),1), BullardResults'];
    
            % Convert the 1x18 string array to a table row
            newRow = array2table(rowData, 'VariableNames', varNames);
            newRowBullRes = array2table(rowBullRes, 'VariableNames',varNamesBullRes) ;
    
            % Append the new row to the main table
            outputTable = [outputTable; newRow];
            outputTableBullRes = [outputTableBullRes; newRowBullRes];
        ...
        filenameBullRes = [pwd '/outputs/BullardResultsData_batch_' currentDateTimeString '.xlsx'];
        % Write the table to the Excel file
        writetable(outputTable, filename);
        writetable(outputTableBullRes, filenameBullRes);
    ...
    %%
    function [HFData, ErrData,BullardResults]=batchmode(filePath, PulseData,folderPath,change_pars,par_file, counter)
    ...
    %% Print results to results (.res) file
                ...
                % ----------------------------------------------
                BullardResults=PrintBullardResults(ResFileId, ...
                Iteration, ...
                Trial, ...
                PulseData, ...
                TChange, ...
